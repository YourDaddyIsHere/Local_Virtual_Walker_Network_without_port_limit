...
class Simulation(DatagramProtocol):
    ...
    def generate_blocks(self,node):
        ...
        if self.attack_edges.has_node(node.id):
            ...
            link_generator_block_of_this_node = LinkNumberGenerator(number_of_nodes=self.honest_node_number+self.evil_node_number,number_of_link=self.link_per_node,
                                                  link_range=self.link_range,upload_cap=self.upload_cap,download_cap=self.download_cap,
                                                  introduction_seed=int(node.id)+20,block_seed=int(node.id)+30)
            data = link_generator_block_of_this_node.get_current()
